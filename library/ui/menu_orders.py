...
def manage_order_items(order_id):
    while True:
        ...
        if choice == "1":
            ...
            # Можно получить позицию, чтобы проверить
            # Для этого потребуется функция получения по ID, но у нас есть get_items_by_order_id()
            # и в модели OrderItem есть id
            # Сделаем так:
